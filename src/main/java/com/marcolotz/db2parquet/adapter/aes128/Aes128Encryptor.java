...
public class Aes128Encryptor implements Encryptor {
  ...
  private byte[] encryptGCM(AesKey.ExpandedKey key, byte[] txt)
    throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, NoSuchProviderException {
  ...
  byte[] decryptGCM(AesKey.ExpandedKey key, byte[] encryptedMessage)
    throws NoSuchAlgorithmException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, InvalidAlgorithmParameterException, NoSuchPaddingException {
    ...
    final Cipher cipher = Cipher.getInstance(AES_GCM_CIPHER_LBL);
    cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key.encKey, "AES"),
      new GCMParameterSpec(key.keySize.getKeySizeBits(), iv));
